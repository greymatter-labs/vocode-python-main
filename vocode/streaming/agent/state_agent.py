...
class StateAgent(RespondAgent[CommandAgentConfig]):
    ...
    async def generate_completion(
        ...
        self.logger.info(
            f"[CallType.{self.agent_config.call_type}:{self.agent_config.current_call_id}] Lead:{human_input}"
        )
    ...
    async def compose_action(self, state):
        ...
        async def run_action_and_return_input(action, action_input):
            ...
            self.logger.info(
                f"[CallType.{self.agent_config.call_type}:{self.agent_config.current_call_id}] Agent: {pretty_function_call}"
            )
