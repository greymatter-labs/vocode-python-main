...
class GetNextBoulevardAppointmentResponse(BaseModel):
    appointment_info: Optional[Dict[str, str]] = Field(None, description="Information about the next appointment")
...
class GetNextBoulevardAppointment(
    ...
    parameters_type: Type[GetNextBoulevardAppointmentParameters] = GetNextBoulevardAppointmentParameters
    response_type: Type[GetNextBoulevardAppointmentResponse] = GetNextBoulevardAppointmentResponse
    ...
    async def run(
        ...
        try:
            next_appointment = await retrieve_next_appointment_by_phone_number(
                self.action_config.phone_number, self.action_config.timezone
            )
            ...
            if next_appointment:
                appointment_info = {
                    ...
                    "services": ", ".join(
                        [service["service"]["name"] for service in next_appointment["appointmentServices"]]
                    ),
            ...
            logger.error(f"An error occurred while retrieving the next appointment: {str(e)}")
            return ActionOutput(
                action_type=action_input.action_config.type,
                response=GetNextBoulevardAppointmentResponse(
                    appointment_info=None
